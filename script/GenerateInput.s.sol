 ...
 * @title GenerateInput
 * @author Loc Giang
 * @notice This script generates a JSON input file for creating a Merkle tree for an airdrop.
 * It creates a standardized input file that will be used by another script (MakeMerkle.s.sol) to generate
 * Merkle trees and proofs
 */
contract GenerateInput is Script {
    // 25 tokens per person
    uint256 private constant AMOUNT = 25 * 1e18;

    string[] types = new string[](2);
    uint256 count;

    // 4 eligible addresses
    ...
    /**
     * _createJSON() function builds a JSON string with:
     * types: Data types for each field (["address", "uint"])
     * count: Number of recipients (4)
     * values: Object containing each recipient's data
     */
